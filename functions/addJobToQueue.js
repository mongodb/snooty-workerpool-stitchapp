exports = function(payload, jobTitle, jobUserName, jobUserEmail, collectionName){
  console.log(JSON.stringify(context.user));
  
  const db_name = context.values.get("db_name");
  
  // get the queue collection
  var queue = context.services.get("mongodb-atlas").db(db_name).collection(collectionName);
  
  // create the new job document
  const newJob = {
    title: jobTitle,
    user: jobUserName, 
    email: jobUserEmail,
    status: "inQueue",
    createdTime: new Date(),
    startTime: null, 
    endTime: null, 
    priority: 1,
    error: {}, 
    result: null, 
    payload: payload, 
    logs: [],
  };
  
  // we are looking for jobs in the queue with the same payload that have not yet started (startTime == null)
  const filterDoc = {payload: payload, status: { $in: ["inQueue", "inProgress"]}};
  const updateDoc = {$setOnInsert: newJob};
  
  // upsert the new job and return if the upsertedId if the job was added to the queue
  return queue.updateOne(filterDoc, updateDoc, {upsert: true}).then(result => {
    if (result.upsertedId) {
      console.log("it is now entered! ", result.upsertedId)
      return result.upsertedId;
    } else {
      console.log("it already exists!!")
      return "Already Existed";
    }}, (error) => {
      return error;
    });
};